...
// src/components/FeedbackCards.js
import React, { useState, useEffect } from 'react';
import {
  Container,
  Card,
...
const FeedbackCards = () => {
  ...
  // Generate gradient background from name
  const stringToColor = (string = 'User') => {
    let hash = 0;
    for (let i = 0; i < string.length; i++) {
      hash = string.charCodeAt(i) + ((hash << 5) - hash);
    }
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 8, mt: '80px' }}>
      <Typography
        variant="h3"
        align="center"
        sx={{
      ...
      {loading ? (
        ...
        <Grid container spacing={4} justifyContent="center">
          {feedbacks.map((fb) => (
            <Grid item xs={12} sm={6} md={4} key={fb._id} sx={{ display: 'flex' }}>
              <Card
                sx={{
                  borderRadius: 4,
                  width: '100%',
              ...
              >
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                    <Avatar
                      sx={{
                        background: stringToColor(fb.name),
                        width: 56,
                        height: 56,
                        fontWeight: 'bold',
                        fontSize: '1.2rem',
                    ...
                    <Box>
                      <Typography variant="subtitle1" fontWeight="600" sx={{ color: '#1a237e' }}>
                        {fb.name || 'Anonymous'}
                      </Typography>
                      <Typography
                        variant="body2"
                        sx={{ color: 'text.secondary', display: 'flex', alignItems: 'center', mt: 0.5 }}
                      >
